...
public class BetterPathFinder : MonoBehaviour
{
    ...
    public bool isPathRigid;
    public List<GameObject> objectsOnTheGrid;
    public Stack<int[]> pathToFollow;
    ...
    public void createPathfinder()
    {
        ...
        while (stage == 1)
        {
            ...
            if (temp.Count == 1)
            {
                ...
                //-----
                pathToFollow.Push(new int[] { (int)Mathf.Floor(coords[0] - maxPathLength + pos.x), ((int)Mathf.Floor(coords[1] - maxPathLength + pos.z)) });
                //-----
            }
            if (temp.Count >= 2)
            {
                ...
                if (isPathRigid)
                {
                    coords = temp.ToArray()[0];
                }
                else
                {
                    coords = temp.ToArray()[(int)Mathf.Floor(Random.Range(0, temp.Count))];
                }
